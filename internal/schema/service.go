...
var ServiceSchema = &schema.BodySchema{
	Attributes: map[string]*schema.AttributeSchema{
		...
		// TODO: add examples
		"port": {
			Description: lang.Markdown("Specifies the port to advertise for this service. The value of `port` depends on which [`address_mode`](/nomad/docs/job-specification/service#address_mode) is being used:\n\n- [`alloc`](/nomad/docs/job-specification/service#alloc) - Advertise the mapped `to` value of the labeled port and the allocation address. If a `to` value is not set, the port falls back to using the allocated host port. The `port` field may be a numeric port or a port label specified in the same group's network block.\n- [`alloc_ipv6`](/nomad/docs/job-specification/service#alloc_ipv6) - Same as `alloc` but use the IPv6 address in case of dual-stack or IPv6-only.\n- [`driver`](/nomad/docs/job-specification/service#driver) - Advertise the port determined by the driver (e.g. Docker). The `port` may be a numeric port or a port label specified in the driver's `ports` field.\n- [`host`](/nomad/docs/job-specification/service#host) - Advertise the host port for this service. `port` must match a port _label_ specified in the [`network`](/nomad/docs/job-specification/network) block."),
			Constraint:  &schema.LiteralType{Type: cty.String},
			IsOptional:  true,
		},
		"tags": {
		...
		"enable_tag_override": {
			Description:  lang.Markdown("Enables users of Consul's Catalog API to make changes to the tags of a service without having those changes be overwritten by Consul's anti-entropy mechanism. See Consul [documentation](/consul/docs/concepts/anti-entropy#enable-tag-override) for more information. Only available where `provider = \"consul\"`."),
		...
		"tagged_addresses": {
			Description: lang.Markdown("Specifies custom [tagged addresses](/consul/docs/discovery/services#tagged-addresses) to advertise in the Consul service registration. Only available where `provider = \"consul\"`."),
			Constraint:  &schema.LiteralType{Type: cty.Map(cty.String)},
			IsOptional:  true,
		},
		"address_mode": {
			Description:  lang.Markdown("Specifies which address (host, alloc, alloc_ipv6 or driver-specific) this service should advertise. See [below for examples.](/nomad/docs/job-specification/service#using-driver-address-mode) Valid options are:\n\n- [`alloc`](/nomad/docs/job-specification/service#alloc-1) - For allocations which create a network namespace, this address mode uses the IP address inside the namespace. Can only be used with \"bridge\" and \"cni\" [networking modes](/nomad/docs/job-specification/network#mode). A numeric port may be specified for situations where no port mapping is necessary. This mode can only be set for services which are defined in a \"group\" block.\n- [`alloc_ipv6`](/nomad/docs/job-specification/service#alloc_ipv6-1) - Same as `alloc` but use the IPv6 address in case of dual-stack or IPv6-only.\n- [`auto`](/nomad/docs/job-specification/service#auto) - Allows the driver to determine whether the host or driver address should be used. Defaults to `host` and only implemented by Docker. If you use a Docker network plugin such as weave, Docker will automatically use its address.\n- [`driver`](/nomad/docs/job-specification/service#driver-1) - Use the IP specified by the driver, and the port specified in a port map. A numeric port may be specified since port maps aren't required by all network plugins. Useful for advertising SDN and overlay network addresses. Task will fail if driver network cannot be determined. Only implemented for Docker. This mode can only be set for services which are defined in a \"task\" block.\n- [`host`](/nomad/docs/job-specification/service#host-1) - Use the host IP and port."),
	...
	Blocks: map[string]*schema.BlockSchema{
		...
		"identity": {Description: lang.PlainText("Specifies a Workload Identity to use when obtaining Service Identity tokens from Consul to register the service. Only available where `provider = \"consul\"`. Typically this can be omitted so that Nomad will fall back to the server's [`consul.service_identity`](/nomad/docs/configuration/consul#service_identity) block."),
			Body: IdentitySchema,
		},
	},
}
