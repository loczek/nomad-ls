...
var ExecSchema = &schema.BodySchema{
	Attributes: map[string]*schema.AttributeSchema{
		...
		"no_pivot_root": {
			Description:  lang.Markdown("Defaults to `false`. When `true`, the driver uses `chroot` for file system isolation without `pivot_root`. This is useful for systems where the root is on a ramdisk."),
