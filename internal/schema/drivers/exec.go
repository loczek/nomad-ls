...
var ExecDriverSchema = &schema.BodySchema{
	Attributes: map[string]*schema.AttributeSchema{
		...
		"command": {
			Description: lang.Markdown("The command to execute. Must be provided. If executing a binary that exists on the host, the path must be absolute and within the task's [chroot](/nomad/docs/deploy/task-driver/exec#chroot) or in a [host volume](/nomad/docs/configuration/client#host_volume-block) mounted with a [`volume_mount`](/nomad/docs/job-specification/volume_mount) block. The driver will make the binary executable and will search, in order:\n\n- The `local` directory with the task directory.\n- The task directory.\n- Any mounts, in the order listed in the job specification.\n- The `usr/local/bin`, `usr/bin` and `bin` directories inside the task directory.\n\nIf executing a binary that is downloaded from an [`artifact`](/nomad/docs/job-specification/artifact), the path can be relative from the allocation's root directory."),
